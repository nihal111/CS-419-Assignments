...
def gradientDescent(X, Y, W, N, _eta, lamb, degree=1, maxIterations=250):
...
if __name__ == '__main__':
    ...
    for a in xrange(0, len(classifiers)):
        feature = [int(np.ceil(float(x))) for x in train_data[classifiers[a]]]
        classifier_lists.append(list(set(feature)))
        for val in classifier_lists[a]:
            temp = np.copy(np.array(feature))
            indices = temp != val
            temp[indices] = 0
            indices = temp == val
            temp[indices] = 1
    ...
    for i in xrange(0, len(test_data['id'])):
        ...
        for a in xrange(0, len(classifiers)):
            for val in classifier_lists[a]:
                if (int(np.ceil(float(test_data[classifiers[a]][i]))) == val):
                    Q.append(1)
                else:
                    Q.append(0)
